...
* - Какой из принципов IaaC является основополагающим?
    - Идемпотентность (свойство объекта или операции давать тот же результат при применении на другие объекты) - например, в рамках IaaC, код используемый для развертывания серверов/конфигурации серверов, постоянно дает одинаковый результат и конечное состояние. Все настроенные серверы обладают одинаковой конфигурацией(конфиги, переменные, выделенные аппаратные ресурсы и т.д.) Если операция выполняется над объектом повторно, то ничего не происходит, система остается в прежнем состоянии (например разница в применение скрипта bash и ansible - скрипт bash будет выполняться всегда, но в тоже время, система ansible, обладающая свойством идемпотентности, увидит, что объект находится в правильной конфигурации и не будет повторно выполнять действия по конфигурированию).
