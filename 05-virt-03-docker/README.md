...
* - MongoDB, как основное хранилище данных для java-приложения;
    > На мой взгляд, для данной задачи подойдет как Docker c persistent volume (удобно для управления, например при обновлении версии), так и виртуализация(напр. с использованием VMware vSAN). В любом случае придется позаботиться о высокой доступности и отказоуйсточивости "тома" на котором расположится БД.
* - Gitlab сервер для реализации CI/CD процессов и приватный (закрытый) Docker Registry.
