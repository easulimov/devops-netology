...
7. Найдите информацию о том, что такое `:(){ :|:& };:`. Запустите эту команду в своей виртуальной машине Vagrant с Ubuntu 20.04 (**это важно, поведение в других ОС не проверялось**). Некоторое время все будет "плохо", после чего (минуты) – ОС должна стабилизироваться. Вызов `dmesg` расскажет, какой механизм помог автоматической стабилизации. Как настроен этот механизм по-умолчанию, и как изменить число процессов, которое можно создать в сессии?
  ### Решение:
  * `:(){ :|:& };:` - это так называемая forkbomb. Форк-бомба - это форма атаки типа «отказ в обслуживании» (DoS) на систему на базе Linux или Unix. Она использует операцию вилки. `: () {: |: &} ;:` - не что иное, как функция bash. Эта функция выполняется рекурсивно(создаст еще два себя, бесконечно в фоновом режиме). Хотя эта функция сама определена в фоновом режиме, она запускается на переднем плане.
  * `dmesg -HT` - показал:
  ```
  ...
  ```
  * По умолчанию этот механизм настроен на ограничение максимального количества запускаемых пользователем процессов.
